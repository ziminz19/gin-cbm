...
if __name__ == '__main__':
    ...
    pyg_dataset = PygGraphPropPredDataset(name = 'ogbg-molesol')
    def add_attributes(dataset):
        data_list = []
        for i, data in enumerate(dataset):
            data.new_concept = torch.Tensor([i, 1, 5, i])
            data_list.append(data)
        dataset.data, dataset.slices = dataset.collate(data_list)
        return dataset
    dataset = add_attributes(pyg_dataset[0:10])
    print(dataset.num_tasks)
    #print([pyg_dataset[i].new_concept for i in range(10)])
